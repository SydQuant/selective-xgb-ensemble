...
from metrics.simple_metrics import information_ratio, turnover as sig_turnover
...
def weight_objective_factory(
    ...
    def f(theta: Dict[str, float]) -> float:
        ...
        # Use objective_fn if provided, otherwise use legacy approach
        ...
        to = sig_turnover(s)
        val -= turnover_penalty * to
        
        # p-value gate on the chosen DAPY metric (legacy approach)
        if metric_fn_dapy is not None and objective_fn is None:
