"""
Export TY (@TY#C) raw data from ArcticDB database.

This script connects to the ArcticDB database and exports raw OHLCV data
for the US 10-Year Treasury futures (@TY#C) symbol to CSV format for analysis.
"""
...
def export_ty_raw_data(output_dir=None, start_date=None, end_date=None, format='csv'):
    """
    Export TY (@TY#C) raw data from ArcticDB.
    ...
    try:
        ...
        filename = f"TY_raw_data_{timestamp}{date_suffix}.{format}"
        ...
        logger.error(f"Error exporting TY data: {e}")
        raise
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Export TY (@TY#C) raw data from ArcticDB')
    ...
    try:
        output_path = export_ty_raw_data(
