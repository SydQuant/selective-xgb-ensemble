...
def fit_xgb_on_slice(X_tr: pd.DataFrame, y_tr: pd.Series, spec: Dict[str, Any]):
    ...
    # Scale target by 1000x to help XGBoost learn patterns better
    y_scaled = y_tr * 1000.0
    
    model.fit(X_tr.values, y_scaled.values)
    return model
...
def fold_train_predict(X_tr: pd.DataFrame, y_tr: pd.Series, X_te: pd.DataFrame, specs: List[Dict[str, Any]]):
    ...
    for spec in specs:
        ...
        # Scale predictions back to original scale (divided by 1000)
        p_tr = pd.Series(m.predict(X_tr.values) / 1000.0, index=X_tr.index, name="m")
        p_te = pd.Series(m.predict(X_te.values) / 1000.0, index=X_te.index, name="m")
