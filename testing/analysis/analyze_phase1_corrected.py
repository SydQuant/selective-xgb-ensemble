#!/usr/bin/env python3
"""
Corrected Phase 1 Results Analysis - Extract proper OOS metrics from multi-year data
"""
...
def analyze_phase1_results():
    """Analyze corrected Phase 1 results with proper OOS metrics"""
    
    log_files = {
        '5_folds': "logs/xgb_analysis_p1_05folds_25models_100feat_standard_ES_standard_100feat_5folds_20250905_135341.log",
        '10_folds': "logs/xgb_analysis_p1_10folds_25models_100feat_standard_ES_standard_100feat_10folds_20250905_135346.log", 
        '15_folds': "logs/xgb_analysis_p1_15folds_25models_100feat_standard_ES_standard_100feat_15folds_20250905_135352.log"
    }
    
    results = {}
    
    print("=" * 80)
    print("PHASE 1 CORRECTED ANALYSIS - PROPER OOS METRICS")
    print("=" * 80)
    print("Multi-Year Data (2015-2024): 2324 observations across 10 years")
    print()
    ...
    for config_name, log_file in log_files.items():
        if os.path.exists(log_file):
            ...
            if oos_sharpe_values:
                ...
                # Default hit rate if not found
                if not oos_hit_values:
                    oos_hit_values = [0.5] * len(oos_sharpe_values)  # Assume 50% if not found
                
                avg_hit_rate = statistics.mean(oos_hit_values) if oos_hit_values else 0.5
                ...
                print(f"  Total Models Analyzed: {len(oos_sharpe_values)}")
    ...
    print("=" * 80)
    print(f"{'Metric':<25} | {'5 Folds':<12} | {'10 Folds':<12} | {'15 Folds':<12} | {'Winner':<10}")
    print("-" * 85)
    
    if results:
        configs = ['5_folds', '10_folds', '15_folds']
        ...
        print(f"{'Avg OOS Sharpe':<25} | {sharpe_values.get('5_folds', 0):<12.3f} | {sharpe_values.get('10_folds', 0):<12.3f} | {sharpe_values.get('15_folds', 0):<12.3f} | {sharpe_winner:<10}")
        print(f"{'Avg Hit Rate':<25} | {hit_values.get('5_folds', 0):<12.3f} | {hit_values.get('10_folds', 0):<12.3f} | {hit_values.get('15_folds', 0):<12.3f} | {hit_winner:<10}")
        print(f"{'Sharpe Consistency':<25} | {consistency_values.get('5_folds', 0):<12.3f} | {consistency_values.get('10_folds', 0):<12.3f} | {consistency_values.get('15_folds', 0):<12.3f} | {consistency_winner:<10}")
        print(f"{'Statistical Significance':<25} | {significance_values.get('5_folds', 0):<12.1f}% | {significance_values.get('10_folds', 0):<12.1f}% | {significance_values.get('15_folds', 0):<12.1f}% | {significance_winner:<10}")
        ...
        for config in configs:
            if config in results:
                ...
                if sharpe_values:
                    ...
                    score += 0.25 * (sharpe_values[config] / max_sharpe if max_sharpe > 0 else 0)
                # Hit rate (15% weight)
                if hit_values:
                    max_hit = max(hit_values.values())
                    score += 0.15 * (hit_values[config] / max_hit if max_hit > 0 else 0)
        ...
        if scores:
            ...
            print(f"Phase 1 Winner: {winner} (Score: {scores[winner]:.3f})")
            print()
            for config in sorted(scores.keys(), key=lambda k: scores[k], reverse=True):
                fold_count = config.split('_')[0]
                print(f"  {fold_count}: {scores[config]:.3f}")
